        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                conflicts = conn.execute(
                    "SELECT COUNT(*) FROM fhir_curated_resource WHERE has_conflict=1"
                ).fetchone()[0]
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            bundle = {
                ...
                "entry": [
                    ...
                    {"resource": {"resourceType": "Observation", "id": "obs-1", "status": "final", "code": {"text": "demo"},
            ...
            bundle = {
                ...
                "entry": [
                    ...
                    {"resource": {"resourceType": "Encounter", "id": "enc-1", "status": "finished",
                                  ...
                                  "subject": {"reference": "Patient/pat-1"}}},
                    {"resource": {"resourceType": "Condition", "id": "cond-1", "code": {"text": "demo condition"},
                                  "subject": {"reference": "Patient/pat-1"}}},
                    {"resource": {"resourceType": "Observation", "id": "obs-1", "status": "final", "code": {"text": "demo obs"},
                                  "subject": {"reference": "Patient/pat-1"}}},
                    {"resource": {"resourceType": "StructureDefinition", "id": "sd-1",
                                  "url": "http://example.org/fhir/StructureDefinition/demo", "version": "1.0.0",
                                  "name": "DemoProfile", "status": "draft", "kind": "resource", "abstract": False,
                                  "type": "Patient", "derivation": "constraint", "differential": {"element": []}}},
                    {"resource": {"resourceType": "ValueSet", "id": "vs-1",
                                  "url": "http://example.org/fhir/ValueSet/demo", "version": "1.0.0",
                                  "name": "DemoVS", "status": "draft", "compose": {"include": []}}},
                    {"resource": {"resourceType": "CodeSystem", "id": "cs-1",
                                  "url": "http://example.org/fhir/CodeSystem/demo", "version": "1.0.0",
                                  "name": "DemoCS", "status": "draft", "content": "complete", "concept": []}},
                ],
            }
    ...
    def test_export_bundle_xml_strict_medicationrequest_procedure(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                ex = export_curated_bundle_xml(conn, out_xml, limit=200, mode="strictish")
                self.assertTrue(ex.ok, ex.message)
    ...
    def test_export_bundle_xml_strict_medication_medicationdispense(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                # Ensure primitive value attribute exists for status (ignore XML formatting differences)
                import xml.etree.ElementTree as ET

                root = ET.fromstring(xml_text)
                ns = {"f": "http://hl7.org/fhir"}
                status = root.find(".//f:MedicationDispense/f:status", ns)
                self.assertIsNotNone(status)
                self.assertEqual(status.attrib.get("value"), "completed")
            finally:
                conn.close()
    ...
    def test_strictish_mixed_bundle_does_not_abort(self):
        ...
        mixed = {
            ...
            "entry": [
                ...
                {"resource": {"resourceType": "Procedure", "id": "pr1", "status": "completed",
                              "code": {"text": "demo procedure"},
